...
public class FotoBot extends Application {
    ...
    public String Main_Help = "<h1><font color=white>Фотобот<br>Руководство пользователя</font></h1>" +
            ...
            "через заданные промежутки времени. Первое, что необходимо сделать после установки Fotobot " +
            "это завести для него почтовый ящик. Можно использовать для этого Google Mail (gmail.com) или mail.ru. " +
            "Необходимым условием является одноступенчатая аутентификация, т.е. доступ к почте только с использованием " +
            "имени пользователя и пароля. Никаких подтверждений по SMS не надо. После того, как почтовый ящик для Fotobot " +
            "заведен, необходимо в настройках прописать имя почтового сервера (для Google - это gmail.smtp.com, для mail.ru -" +
            "это smtp.mail.ru) и номер порта (465). Если у вас другой почтовый сервер, то имя smtp сервера и номер порта " +
            "можно найти на странице помощи этого сервера. Эта часть самая сложная в настройке Fotobot, остальные параметры в настройках " +
            "имеют подсказки и просты в конфигурации.";

    /**
     * Если  Show_Help = false, то вывлдится timestamp для логов,
